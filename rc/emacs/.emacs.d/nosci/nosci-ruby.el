...
;--------------------------------------------------
; (eval-when-compile
;   (load-file "nosci-loadpath"))
;-------------------------------------------------- 

(defun nosci-ruby-mode-hook ()
  "Correctly setup a Ruby buffer"
  (require 'ruby-electric)
  (ruby-electric-mode t)
  (inf-ruby-keys)
  (nosci-programming-mode-hook)
  (define-key ruby-mode-map "\C-c\t" 'nosci-ruby-comment-bar)
  (require 'align)
...
(add-hook 'ruby-mode-hook
          (lambda ()
            (nosci-ruby-mode-hook)
            (define-key ruby-mode-map "\t"   'nosci-smart-tab)
            (define-key ruby-mode-map "\C-m" 'nosci-newline)))
...
;; Ruby on Rails
(add-to-list 'load-path (concat nosci-site-lisp "/rinari"))
(require 'rinari)

(defun nosci-rhtml-mode-hook ()
  "Stuff common across all RHTML buffers"
  (nosci-programming-mode-hook)
  (setq fill-column 120))

(add-to-list 'load-path (concat nosci-site-lisp "/rhtml"))
(autoload 'rhtml-mode "rhtml-mode" "RHTML" t)
(add-to-list 'auto-mode-alist '("\\.rhtml$" . rhtml-mode))
(add-to-list 'auto-mode-alist '("\\.html\.erb$" . rhtml-mode))
(add-hook 'rhtml-mode-hook 'nosci-rhtml-mode-hook)
